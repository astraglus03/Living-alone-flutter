...
class SignupSettingPasswordScreen extends ConsumerStatefulWidget {
  static String get routeName => 'setPw';
  const SignupSettingPasswordScreen({super.key});

  @override
  ConsumerState<SignupSettingPasswordScreen> createState() => _SignupSettingPasswordScreenState();
}

class _SignupSettingPasswordScreenState extends ConsumerState<SignupSettingPasswordScreen> {
      ...
      child: Column(
        children: [
          ...
          CustomBottomButton(
            ...
            onTap: () async {
              if (isPwValid) {
                ref.read(signupProvider.notifier).setPassword(passwordController.text);
                context.goNamed(SignupNicknameScreen.routeName);
              } else {
